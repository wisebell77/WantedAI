...
**표시 원칙**:
- **적합도(fit)** 는 `%`가 아니라 **"겹치는 역량 개수"**(예: `겹침 5/8개`)로 보여준다.
  `72%` 는 "72%로 합격"처럼 오해되기 쉬워서다(기획서 원칙: 확정·예측이 아닌 근거 대조).
  정렬에는 연속값(0~1)을 그대로 쓰되, 사용자에겐 셀 수 있는 사실만 노출한다.
- **진행도(coverage)** 는 `%`로 보여준다 — 이건 "합격 확률"이 아니라 *자소서를 얼마나 썼나*라 %가 자연스럽다.
