...
대학생의 경험 입력부터 직무 추천, 실시간 공고, 공고별 AI 코치, 자소서와
면접 연습까지 한 흐름으로 연결한 프론트엔드 통합 브랜치입니다. 내부 패키지명과
저장소명은 기존 팀 코드를 보존하기 위해 그대로 두고, 사용자 화면의 서비스명은
`Next Step`으로 통일했습니다.

## 프론트엔드 실행

팀 공유 데이터의 `data_overlap` 내용을 `data/overlap`에 둔 뒤 실행합니다.
...
브라우저에서 `http://127.0.0.1:4173`을 열면 시작 화면이 표시됩니다.
추천 엔진은 별도 프로세스/컨테이너 계약을 유지하며, 연결하지 않은 상태에서는
명시적인 오류를 표시하고 시연 응답으로 대체하지 않습니다.
