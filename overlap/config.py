...
class Settings:
    ...
    교차평가로 잡았다. 억지 매칭을 버리니 **정확도가 올랐다** —
    없을 때 Top-1 58.4% / Top-3 77.5% → 0.55 에서 60.7% / 80.5%.
    잃은 질의는 373 → 369, 넷뿐이다. 0.60 부터는 멀쩡한 문장까지 버려 나빠진다.

    절대 임계만으로는 '맞는 게 하나도 없는 문장'을 못 거른다. 상위 후보가
    0.03 안에 몰려 있으면 1등도 맞는 게 아닌데, min_similarity 는 그걸 모른다.
    실측에서 골프장 캐디 업무(0.451)와 Matlab(0.449)이 '학회 운영진 경험'의
    근거로 화면에 올라갔다. 억지로 붙이느니 못 읽었다고 하는 편이 낫다."""
