...
const state = {
  ...
  live: [], contexts: [], dataSource:"loading", selected: null, recommendation: null,
...
const daysLeft = (date) => date ? Math.ceil((new Date(`${date}T23:59:59`) - new Date()) / 86400000) : null;
...
function home() {
  ...
  return shell(`<main class="page"><div class="page-head"><div><h1>${state.auth?`${safe(state.auth.id)} 님의`:"미리 보는"} Next Step</h1><p>오늘 해야 할 일과 담아둔 공고를 한눈에 확인해요.</p></div><button class="button secondary" data-go="recommend">+ 경험 추가하기</button></div>
    <div class="home-priority-cards"><section class="panel priority-card next-step-card"><div class="priority-card-head"><span class="priority-card-icon" aria-hidden="true">N</span><span class="priority-card-label">NEXT STEP</span></div><h2>${focus?"오늘의 NEXT STEP":"첫 번째 공고를 담아보세요"}</h2><div class="priority-card-content">${focus
    ...
    <section class="panel priority-card todo-summary-card" id="open-calendar" role="button" tabindex="0" aria-label="오늘의 할 일과 통합 캘린더 열기"><div class="priority-card-head"><span class="priority-card-icon" aria-hidden="true">▦</span><span class="priority-card-label">TODAY</span></div><h2>오늘의 할 일</h2><div class="priority-card-content">${today.items.length
...
function jobs() {
  return shell(`<main class="page"><div class="page-head"><div><h1>실시간 공고</h1><p>실제 수집 공고 중 지금 지원 가능한 공고를 확인하고, 관심 공고를 내 공고함에 담아보세요.</p></div><button class="button secondary" id="manual-posting">+ 공고 직접 등록</button></div>
    <section class="panel featured-jobs"><div class="section-head"><div><p class="eyebrow">FOR YOUR NEXT STEP</p><h2>추천 공고</h2><p>마감이 가깝고 AI 코치까지 연결되는 공고예요.</p></div><span class="badge ${state.dataSource==='api'?'mint':'sun'}">${state.dataSource==='api'?'실제 공고 API':'실제 공고 기반 체험 데이터'}</span></div><div class="posting-grid" id="featured-postings">${state.live.filter(x=>x.coachReady).slice(0,3).map(postingCard).join("")}</div></section>
    <div class="section-head"><div><h2>전체 공고</h2><p>작게 필터링하고, 마음에 드는 공고만 내 공고함에 담아보세요.</p></div></div><div class="filterbar"><input class="input" id="job-search" placeholder="기업명 또는 직무 검색"><select class="select" id="job-filter"><option value="">전체 직무</option>${[...new Set(state.live.map(x=>x.jobFamily))].sort().map(x=>`<option>${safe(x)}</option>`).join("")}</select><select class="select" id="deadline-filter"><option value="0">전체 마감</option><option value="7">7일 이내</option><option value="14">14일 이내</option><option value="30">30일 이내</option></select><span class="result-count">${state.live.length}건</span></div><div class="posting-grid" id="all-postings">${state.live.slice(0,60).map(postingCard).join("")}</div>
  </main>`);
}
function recommend() {
...
function coach() {
  ...
  return shell(`<main class="page"><section class="panel coach-context"><div class="company-logo">${initials(job.companyName)}</div><div><h2>${safe(job.companyName)} · ${safe(job.positionTitle)}</h2><p>${safe(job.jobFamily||"")} · ${dday(job.deadline)}</p></div><a class="button small ghost" href="${safe(job.sourceUrl||'#')}" target="_blank" rel="noreferrer">공고 원문</a></section>
    <div class="coach-grid"><section class="panel"><div class="coach-welcome"><div class="coach-orb">N</div><div><h2>이 공고의 준비 흐름을 함께 정리할게요.</h2><p>공고 JD와 입력한 경험을 근거로 답해요. 우선 자소서에 쓸 경험을 고르고 면접 질문까지 이어가 보세요.</p></div></div><div class="quick-actions"><button class="quick-action" data-agent="timeline">준비 일정 짜기</button><button class="quick-action" data-go="essay">자소서 준비</button><button class="quick-action" type="button" data-interview>면접 연습</button></div>
      ...
      <aside class="panel"><div class="section-head"><div><h2>준비 체크리스트</h2><p>코치의 제안이 홈과 함께 연결돼요.</p></div></div><div class="aside-list" id="coach-tasks">${tasks.length?tasks.map(t=>`<label class="aside-item"><input type="checkbox" ${t.done?'checked':''}> <strong>${safe(t.title)}</strong>${safe(t.reason||"")}</label>`).join(""):`<div class="aside-item">준비 일정을 만들면 할 일이 여기에 정리돼요.</div>`}</div></aside></div></main>`);
}
function essay() {
...
function bindCoach(){const job=state.selected||savedItems()[0];if(!job)return;$('[data-interview]')?.addEventListener('click',()=>{const openPreview=window.NextStepPreview?.openInterview;if(window.location.protocol==='file:'){if(typeof openPreview==='function')openPreview(job);else toast('면접 미리보기를 불러오지 못했어요.');return;}if(typeof openPreview==='function'){openPreview(job);return;}const url=new URL('/index.html',window.location.origin);url.searchParams.set('postingId',job.postingId);window.location.assign(url.href);});const ask=async(mode,message)=>{const box=$('#messages');box.insertAdjacentHTML('beforeend',`<div class="message user">${safe(message)}</div><div class="message assistant" data-wait>공고 근거를 확인하고 있어요…</div>`);const history=historyFor(job.postingId);const tasks=taskItems();const out=await requestCoach({job,mode,message,profile:state.profile,currentTasks:tasks[job.postingId]||[],history});$('[data-wait]').innerHTML=`${out.demo?'<span class="demo-label">체험 응답</span>':''}${safe(out.answer)}`;const next=(out.nextActions||[]).map((x,i)=>({...x,id:`${Date.now()}-${i}`,done:false}));tasks[job.postingId]=next;persistForSession(state.auth?'tasks':'guestTasks',tasks);history.push({role:'user',content:message},{role:'assistant',content:out.answer,demo:Boolean(out.demo)});saveHistory(job.postingId,history);$('#coach-tasks').innerHTML=next.map(t=>`<label class="aside-item"><input type="checkbox"> <strong>${safe(t.title)}</strong>${safe(t.reason||'')}</label>`).join('');box.scrollTop=box.scrollHeight;};$$('[data-agent]').forEach(b=>b.onclick=()=>ask(b.dataset.agent,'마감일까지의 준비 일정을 제안해 줘.'));$('#chat-form').onsubmit=e=>{e.preventDefault();const input=$('#chat-input'),value=input.value.trim();if(value){input.value='';ask('consultation',value);}};}
function bindEssay(){const job=state.selected||savedItems()[0];$('#essay-form').onsubmit=async e=>{e.preventDefault();const sections=$$('.essay-question').map((q,i)=>({question:q.value,answer:$$('.essay-answer')[i].value}));const drafts=draftItems();drafts[job.postingId]=sections;persistForSession(state.auth?'drafts':'guestDrafts',drafts);$('#essay-review').innerHTML='<div class="aside-item">AI 채용담당자가 초안을 읽고 있어요…</div>';const out=await requestEssayReview({job,sections,experiences:(state.profile.experiences||'').split(/\n+/)});const labels={strong:['강점','mint'],weak:['보완 필요','sun'],missing:['추가 작성 필요','coral']};$('#essay-review').innerHTML=`${out.demo?'<div class="alert info">체험 평가 결과예요. 실제 Persona API 연결 시 같은 영역에 결과가 표시됩니다.</div>':''}${out.items.map(x=>`<article class="review-item"><span class="badge ${labels[x.status][1]}">${labels[x.status][0]}</span><h4>${safe(x.label)}</h4><p>${safe(x.feedback)} ${safe(x.suggestion||'')}</p></article>`).join('')}`;};}

async function loadData(){const data=await loadCareerData();state.live=data.postings;state.contexts=data.contexts;state.dataSource=data.source;if(data.source==='fixture')toast('실제 공고 기반 체험 데이터를 불러왔어요.');render();}
